...
import { sendOtpWhatsApp } from "@/lib/services/whatsAppService";
 ...
 *
 * WhatsApp OTP (Module 7):
 *   After the email OTP is sent, sendVerificationOTP also attempts to deliver
 *   the same code via WhatsApp if the student has a mobileNumber in the Student
 *   table. WhatsApp delivery is best-effort — any failure is logged and swallowed
 *   so that email OTP delivery is never blocked.
 *
 * Auth flow by role:
  ...
  plugins: [
    ...
    emailOTP({
      async sendVerificationOTP({ email, otp, type }) {
        // ── 1. Email OTP (authoritative — throw on failure) ────────────────
        try {
          await sendOtpEmail({ email, otp, type });
        } catch (error) {
          console.error(`[auth] Failed to send ${type} OTP email to ${email}:`, error);
          throw error; // Re-throw: email delivery is required
        }

        // ── 2. WhatsApp OTP (best-effort — never block email delivery) ────
        // Look up the student's phone number. We only do this for student
        // sign-in flows; admin OTP types don't have a Student row.
        try {
          ...
          if (student?.mobileNumber) {
            await sendOtpWhatsApp(student.mobileNumber, otp);
            console.log(`[auth] WhatsApp OTP sent to ${student.mobileNumber} for ${email}`);
          }
        } catch (waError) {
          // Log but do NOT re-throw — email OTP already succeeded.
          console.error(`[auth] WhatsApp OTP failed for ${email} (non-fatal):`, waError);
        }
      },
      otpLength: 6,
